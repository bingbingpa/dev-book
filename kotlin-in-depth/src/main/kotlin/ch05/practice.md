...
3. 함수 타입이란 무엇인가? 코틀린 함수 타입과 자바 함수형 인터페이스의 차이점은 무엇인가?
   ...
   - 코틀린은 왜 SAM 인터페이스로 암시적으로 변환 할 수 없을까? 
     - 코틀린은 람다가 형이다. 그래서 형대신 람다를 쓸 수 있다. 자바에서는 람다가 형이 아니다. 
...
5. 람다와 익명 함수의 차이는 무엇인가? 람다를 쓰는 것보다 익명 함수를 쓰는 것이 더 좋은 때는 언제인가?
   ...
   - **일반적인 람다는 레이블 리턴을 사용하고, 인라인 람다는 상위함수로 리턴되고, 이때는 레이블 리턴을 사용하지 않아도 된다.**
   - 익명함수는 그냥 리턴이 지역적인 리턴이고, 레이블 리턴은 외부 리턴이 된다.
...
7. 호출 가능 참조란 무엇인가? 호출 가능 참조의 여러 가지 형태를 설명하라. 각각의 형태를 자바의 메서드 참조와 비교하라.
   ...
   - **코틀린 리플렉션은 컴파일 타임에 미리 다 정의해 놓기 때문에 리플렉션을 사용한다고 느려지지 않는다. 자바는 리플렉션을 사용하면 런타임에 해석하기 때문에 느려진다.**
...
9. 함수 타입 파라미터의 디폴트 인라인 모드와 noinline, crosssinline 모드를 서로 비교하라.
   ...
   - **바깥쪽이 inline 이 아닌 영역 안에서 inline 을 써도 inline 되지 않는다.**
     - iterator 는 forEach 람다(inline) 보다는 for(i in list) 를 사용하자.
...
15. 영역 함수란 무엇인가? 어떤 일을 처리할 때 어떤 영역 함수를 사용할지 판단하는 기준이 무엇인지 설명하라.
    ...
    - **뭘 쓸것인가???** 
      - 람다의 리턴값을 얻고 싶다면 run, let 을 사용. 내부에서 it 을 쓸거면 let, this 를 쓸거면 run
      - 수신 객체를 얻고 싶다면 also, apply 사용. it 을 사용하려면 also, this 를 사용하려면 apply