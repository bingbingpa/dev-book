 # 만들면서 배우는 클린 아키텍처
- *톰 홈버그 저/박소은 역 | 위키북스*
- [소스](https://github.com/wikibook/clean-architecture)
...
- 비즈니스 규칙 검증하기
  ...
  - **스터디 내용**
    - Q: 엔티티에서 검증하는 메소드를 넘기고 유즈케이스에서 값을 넘겨 검증하게 해서 검증의 책임을 엔티티에 넘기면 되지 않을까?
    - A: 디비를 통해 어떤 값을 가져올지도 결국은 검증의 과정인데 그것을 유즈케이스에서 제외할 수는 없다. 결국 순수하게 엔티티가 검증의 책임을 가지기는 어렵다고 생각한다.
    - Q: 공통된 것들을 묶어서 클래스를 만들고 상속하는 형태로 가져가는 기준은 무엇인가?
    - A: 중요한건 **변화율**이다. 함부로 추상화하지 말자. 예를 들어 게시판이 여러개이고 공통된 속성이 있다고 함부로 추상화를 했다가는 고도화 되면서 겉잡을 수 없게 된다. 
      - 하지만 변화율을 예측하는건 어렵다.
- 유스케이스마다 다른 출력 모델
...
- 웹 어댑터가 일반적으로 하는 일 
  ...
  7. HTTP 응답을 반환
- 웹 어댑터의 책임: 웹 어탭터의 입력 모델을 유스케이스의 입력 모델로 변환할 수 있다는 것을 검증해야 한다.
...
- 영속성 어댑터의 책임
  ...
  5. 출력을 반환한다.
- **모든 데이터베이스 연산을 하나의 리조피토리 인터페이스에 넣어 두면 각 서비스는 인터페이스에서 단 하나의 메서드만 사용하더라도 하나의 '넓은' 포트 인터페이스에 의존성을 갖게 된다.**

## 07 아키텍처 요소 테스트하기

- 여러 개의 단위와 단위를 넘는 경계, 아키텍처 경계, 시스템 결계를 결합하는 테스트는 만드는 비용이 더 비싸지고, 실행이 더 느려지며 깨지기 더 쉬워진다.
- 단위 테스트는 일반적으로 하나의 클래스를 인스턴스화하고 해당 클래스의 인터페이스를 통해 기능들을 테스트한다.
- 통합 테스트는 열결된 여러 유닛을 인스턴스화하고 시작점이 되는 클래스의 인터페이스로 데이터를 보낸 후 유닛들의 네트워크가 기대한대로 잘 동작하는지 검증한다.
...
- **"테스트가 이 버그를 왜 잡지 못했을까?"를 생각하고 이에 대한 답변을 기록하고, 이 케이스를 커버할 수 있는 테스트를 추가해야한다.
시간이 지나면 이 작업들이 배포할 때 마음을 편하게 해줄 것이고, 남겨둔 기록은 시간이 지날수록 상황이 개선되고 있음을 증명해줄 것이다.**

## 08 경계 간 매핑하기

- '매핑하지 않기' 전략
  - 웹 컨트롤러, 유스케이스 계층이 같은 모델을 사용한다. 영속성 계층과 애플리케이션 계층도 같은 관계다.
  - 하나의 모델을 사용하기 때문에 웹, 애플리케이션, 영속성 계층과 관련된 이유로 인해 변경돼야 하기 때문에 **단일 책임 원칙**을 위반한다.
...
- '양방향' 매핑 전략
  ...
  - 단점: 너무 많은 보일러플레이트 코드가 생긴다.
- '완전' 매핑 전략
  - 각 연산마다 별도의 입출력 모델을 사용한다. 이런 모델을 가리켜 커맨드(command), 요청(request) 혹은 빗스한 단어로 표현한다.
  - 각 유스케이스는 전용 필드와 유효성 검증 로직을 가진 전용 커맨드를 가진다.
  - 이 매핑 전략을 전역 패턴으로 추천하지는 않는다.
  - 이 전략은 웹계층과 애플리케이션 계층 사이에서 상태 변경 유스케이스의 경계를 명확하게 할 때 가장 빛을 발한다. 
  - 애플리케이션 계층과 영속성 계층 사이에서는 매핑 오버헤드 때문에 사용하지 않는 것이 좋다.
- '단방향' 매핑 전략
  - 이 전략에서는 모든 계층의 모델들이 같은 인터페이스를 구현한다. 이 인터페이스는 관련 있는 특성에 대한 getter 메서드를 제공해서 도메인 모델의 상태를 캡슐화 한다.
- 언제 어떤 매핑 전략을 사용할 것인가?
  - 그때 그때 다르다. 전역적인 규칙은 없다.
  - 가이드 라인
    ...
    - 만약 애플리케이션 계층에서 영속성 문제를 다뤄야 하게 되면 '양방향' 매핑 전략으로 바꿔서 영속성 문제를 영속성 계층에 가둘 수 있게 한다.

## 09 애플리케이션 조립하기

- **모든 의존성은 안쪽으로, 애플리케이션의 도메인 코드 방향으로 향해야 도메인 코드가 바깥 계층의 변경으로부터 안전하다.**
- 유스케이스가 영속성 어댑터를 호출해야 하고 스스로 인스턴스화 한다면 코드 의존성이 잘못된 방향으로 만들어진 것이다.
- 유스케이스는 인터페이스만 알아야 하고, 런타임에 이 인터페이스의 구현을 제공 받아야 한다.
- 스프링의 클래스패스 스캐닝으로 조립하기 단점
  - 클래스에 프레임워크에 특화된 애너테이션을 붙여야 한다는 점에서 침투적이다.
  - 애플리케이션 컨텍스트에 실제로는 올라가지 않았으면 하는 클래스까지 올라갈 수 있다.
- 스프링의 자바 컨피그로 조립하기
